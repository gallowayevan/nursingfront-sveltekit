<script>
	...
	import { extent, permute } from 'd3-array';
	...
	$: yearExtent = extent(data.values || [{ year: 2015 }, { year: 2032 }], (d) => d.year);
	...
	$: colorScheme = quantize(interpolateHcl('#e0f3db', '#084081'), baseYearOrder.length);
	...
	$: if (geoJSON) {
		projection = geoAlbers().rotate([0, 62, 0]).fitSize([width, height], geoJSON[0].geo);
...
<div id="simple-map-container">
									...
									{#each layer.geo.features as feature}
										<path
											...
											stroke-width={layer.name == 'county'
												...
												: 0}
											stroke="#333"
											style="pointer-events:{mapYearData.has(+feature.properties.id)
												? 'all'
												: 'none'};"
											d={path(feature)}
	...
	{:else}
		<div class="notification">
			Choose a combination of selections and click "Show" to see a map of the model's projections.
		</div>
	{/if}
</div>
