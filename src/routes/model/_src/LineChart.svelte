<script>
	...
	$: xExtent = flatData.length > 0 ? extent(flatData, (d) => d.year) : [2015, 2032];
	...
	{#if data.length > 0}
		...
		<svg id="line-chart-svg" viewBox="0 0 {width} {height}">
			<g class="chart-container">
				...
				<g class="xAxis is-size-6" transform="translate(0,{height - margin.bottom})">
					{#each xTicks as tick (tick)}
						<XTick position={[x(tick), 0]} value={tick} duration={transitionDuration} />
					{/each}
				</g>
				...
				<text
					class="is-size-5"
					text-anchor="middle"
					transform="translate({(width - margin.left - margin.right) / 2 + margin.left},{height -
						10})"
				>
					Year
				</text>
